...
class StickerMessageView(context: Context, attributeSet: AttributeSet?) : LinearLayout(context, attributeSet) {
    ...
    init {
        View.inflate(context, R.layout.sticker_message_view, this@StickerMessageView)
        bindViews()

        attributeSet?.let {
            theme.obtainStyledAttributes(attributeSet, R.styleable.StickerMessageView, 0, 0).apply {
                try {
